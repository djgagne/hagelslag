...
class ModelGrid(object):
    ...
    def load_data(self):
        ...
        if self.file_objects[0] is None:
            raise IOError()
        file_object0 = self.file_objects[0]
        var_name, z_index = self.format_var_name(self.variable, list(file_object0.variables.keys()))
        ntimes = 0
        if 'time' in file_object0.variables[var_name].dimensions:
            ntimes = len(file_object0.dimensions['time'])
        # Multiple times
        if ntimes > 1:
            if z_index is None:
                data = file_object0.variables[var_name][self.forecast_hours].astype(np.float32)
            else:
                data = file_object0.variables[var_name][self.forecast_hours, z_index].astype(np.float32)
        # one time
        else:
            y_dim, x_dim = file_object0.variables[var_name].shape[-2:]
        ...
        if hasattr(file_object0.variables[var_name], "units"):
            units = file_object0.variables[var_name].units
        return data, units
